...
public enum ErrorCode {
  ...
  JUDGE_MODEL_INACTIVE(
      422, "JUDGE_MODEL_INACTIVE", "Judge model must be active before starting an evaluation run."),
  CURL_PARSE_ERROR(
      HttpStatus.BAD_REQUEST.value(),
      "CURL_PARSE_ERROR",
  ...
  DATASET_NOT_FOUND(
      HttpStatus.NOT_FOUND.value(), "DATASET_NOT_FOUND", "No dataset found with the provided ID."),
  DATASET_APPROVAL_INVALID(
      422,
      "DATASET_APPROVAL_INVALID",
      "Dataset cannot be approved with the current test case count."),
  DATASET_ARCHIVED(
      HttpStatus.CONFLICT.value(), "DATASET_ARCHIVED", "Archived dataset cannot be modified."),
  TEST_CASE_NOT_FOUND(
      HttpStatus.NOT_FOUND.value(),
      "TEST_CASE_NOT_FOUND",
  ...
  RUBRIC_GENERATION_FAILED(
      422, "RUBRIC_GENERATION_FAILED", "AI could not generate a valid rubric preview."),

  // ── Evaluation ──
  DATASET_NOT_APPROVED(
      422, "DATASET_NOT_APPROVED", "Dataset must be APPROVED before starting an evaluation run."),
  DATASET_NO_ACTIVE_CASES(
      422, "DATASET_NO_ACTIVE_CASES", "Dataset must have at least one active test case."),
