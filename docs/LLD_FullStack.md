...
## 2. Backend Services LLD (`apps/api/`)
...
1. **Security / API Contract Test:**
   ...
   - Bắn Request `GET /api/projects` không kèm Bearer Token → Expect `HTTP 401 Unauthorized`.
   - Bắn Request kèm token của User A nhưng truy xuất Project của User B → Expect `HTTP 403 Forbidden`.
